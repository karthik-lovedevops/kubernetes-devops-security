pipeline {
  ...
  environment {
    ...
    imageName = "karthikdevopscloud1984/numeric-app:${GIT_COMMIT}"
  ...
  stages {
      ...
      steps {
        sh 'docker run --rm -v $(pwd):/project openpolicyagent/conftest test --policy opa-conf-k8s-security.rego k8s_deployment_service.yaml'
      }
    }
